...
export const WalletStep: React.FC<WalletStepProps> = ({ username, walletStep, loading }) => {
  const checklist = [
    "E-mail verificado",
  ...
  return (
    <View className="flex-1 px-[22px] pb-6 pt-11">
      <View className="min-h-[38px] justify-center">
        <Text className="ml-auto font-mono-semibold text-[9px] tracking-[1px] text-ink-mute">3 / 3</Text>
      </View>
      ...
      <View className="flex-1 items-center justify-center pt-3">
        ...
        <Text className="mt-2.5 max-w-[270px] text-center font-sans text-[13px] leading-5 text-ink-dim">
          Sua carteira Solana criada com segurança. Nada pra anotar, guardamos com criptografia.
        </Text>
      </View>
      ...
      <View className="mb-5 gap-[9px]">
        {checklist.map((item, index) => {
          ...
          return (
            <View key={item} className="flex-row items-center gap-[11px] rounded-[12px] border border-line bg-bg-2 p-3">
              <View
                className={[
                  "h-5 w-5 items-center justify-center rounded-full border",
                  done ? "border-green bg-green" : isLoading ? "border-orange bg-transparent" : "border-line bg-bg-elev",
                ].join(" ")}
              >
                {done ? <Feather name="check" size={12} color="#0a0a0a" /> : null}
                {isLoading ? <ActivityIndicator size="small" color="#ff6b3d" /> : null}
              </View>
              <Text className={`flex-1 font-sans-medium text-[12px] ${done || isLoading ? "text-ink" : "text-ink-mute"}`}>
                {item}
              </Text>
            </View>
          );
      ...
      <View className="flex-row items-center justify-center gap-1.5">
        ...
        <Text className="font-mono text-[9px] text-ink-mute">
          carteira · <Text className="text-ink-dim">{WALLET_ADDRESS_PREVIEW}</Text>
        </Text>
      </View>
    </View>
  );
