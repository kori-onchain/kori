...
import { ArrowRightIcon, CardIcon, InvestIcon } from "@components/layout/icons";
  ...
  >
    ...
    {card.network === "mastercard" ? (
      <View style={styles.badgeMcWrapper}>
        <View
          style={[
            styles.badgeMcCircle,
            { backgroundColor: "#EB001B", left: 0 },
          ]}
        />
        <View
          style={[
            styles.badgeMcCircle,
            { backgroundColor: "#F79E1B", right: 0 },
          ]}
        />
      </View>
    ) : (
...
const Row: React.FC<{ item: Transaction; isLast: boolean }> = ({
  ...
  const card = item.cardId
    ? MOCK_CARDS.find((c) => c.id === item.cardId)
    : undefined;
  ...
  return (
    ...
    >
      ...
      <View style={styles.amounts}>
        <Text style={[styles.amount, { color: isIncoming ? t.green : t.ink }]}>
          {item.amount}
        </Text>
...
export const Transactions: React.FC<TransactionsProps> = ({
  ...
  const { t } = useTheme();
  const items = (transactions || MOCK_TRANSACTIONS).slice(0, PREVIEW_COUNT);

  return (
    <View style={styles.wrapper}>
      <View style={styles.headerRow}>
        <Text style={[styles.sectionTitle, { color: t.ink }]}>Transações</Text>
        <TouchableOpacity
          onPress={onSeeAll}
          activeOpacity={0.7}
