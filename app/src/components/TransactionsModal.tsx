...
export interface TransactionItem {
  ...
  rawAmount: number;
  isInflow: boolean;
  type: 'transfer' | 'deposit' | 'withdraw' | 'invest' | 'card' | 'exchange' | 'cc' | 'other';
  initials?: string;
  isAnonymous?: boolean;
}
...
export const TransactionsModal: React.FC<TransactionsModalProps> = ({ visible, onClose }) => {
  ...
  const [activeDropdown, setActiveDropdown] = useState<'month' | 'category' | 'type' | null>(null);
  ...
  const getCategoryLabel = () => {
    ...
    return CATEGORY_FILTERS.find(c => c.id === activeCategory)?.label || 'Categorias';
  };

  const getTypeLabel = () => {
    if (activeType === 'todos') return 'Tipos de transação';
    return TYPE_FILTERS.find(t => t.id === activeType)?.label || 'Tipos de transação';
  };
  ...
  const filteredGroups = INITIAL_GROUPS.map(group => {
    const filteredTx = group.transactions.filter(tx => {
      ...
      const matchesMonth = activeMonth === 'todos' || group.month.toLowerCase() === activeMonth;
      const matchesCategory = activeCategory === 'todas' || tx.type === activeCategory;
      const matchesType = activeType === 'todos' || (activeType === 'inflow' ? tx.isInflow : !tx.isInflow);
  ...
  const renderGroupItem = ({ item: group, index }: { item: DayGroup; index: number }) => {
